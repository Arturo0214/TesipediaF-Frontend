...
const NIVEL_MAP = { nivel_1: 'Licenciatura', nivel_2: 'Maestría', nivel_3: 'Doctorado', nivel_4: 'Preparatoria', nivel_5: 'Especialidad', nivel_6: 'Diplomado' };
...
const LABEL_MAP = {
  ...
  nivel_1: 'Licenciatura', nivel_2: 'Maestría', nivel_3: 'Doctorado', nivel_4: 'Preparatoria', nivel_5: 'Especialidad', nivel_6: 'Diplomado',
  ...
  const handleGenerateQuotePDF = async () => {
    ...
    try {
      ...
      const pdfBody = JSON.stringify({
        ...
        tipoTrabajo: f.tipoTrabajo === 'Otro' ? (f.tituloTrabajo || f.tema || 'Proyecto académico') : (f.tipoTrabajo || 'Tesis'),
        customTipoTrabajo: f.tipoTrabajo === 'Otro' ? (f.tituloTrabajo || f.tema || '') : '',
        ...
        fechaEntrega: fechaEntregaLegible,
                      ...
                      <Row className="g-2">
                        ...
                        <Col xs={12}>
                          ...
                          <Form.Select size="sm" value={quoteFields.esquemaTipo || '33-33-34'} onChange={(e) => {
                            ...
                            if (tipo === 'personalizado' && (!quoteFields.pagosCustom || !quoteFields.pagosCustom.length)) {
                        ...
                        {quoteFields.esquemaTipo === 'personalizado' ? (
                          <>
                            {(quoteFields.pagosCustom || []).map((pago, i) => (
                              <Col xs={12} key={i}>
                                <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: 4 }}>
                                  <div style={{ flex: 1 }}>
                                    <div className="wq-micro-label">Monto {i + 1}</div>
                                    <div className="input-group input-group-sm">
                                      <span className="input-group-text">$</span>
                                      <Form.Control type="number" min="0" value={pago.monto}
                                        onChange={(e) => {
                                          const arr = [...(quoteFields.pagosCustom || [])];
                                  ...
                                  <div style={{ flex: 1 }}>
                                    <div className="wq-micro-label">Fecha</div>
                            ...
                            <Col xs={12}>
                              {(() => {
                                const total = quoteFields.precioManual ? Number(quoteFields.precioManual) : (quotePrice?.precioBase || 0);
                                const asignado = (quoteFields.pagosCustom || []).reduce((s, p) => s + (Number(p.monto) || 0), 0);
